                    ...
                    @foreach($dataType->readRows as $row)
                        ...
                        <div class="panel-body" style="padding-top:0;">
                            @if($row->type == "image")
                               ...
                               <a href="{{ CDN($dataTypeContent->{$row->field}) }}" target="_blank"> <img class="img-responsive"
                                     src="{{ CDN($dataTypeContent->{$row->field}) }}" width="300"></a>
                            @elseif($row->type == 'multiple_images')
                                @if(json_decode($dataTypeContent->{$row->field}))
                                    @foreach(json_decode($dataTypeContent->{$row->field}) as $file)
        

                                    <a href="{{ CDN($file) }}" target="_blank"> 
                                        <img class="img-responsive"
                                             src="{{ CDN($file) }}" width="300">
                                    </a>
                                    <br>
                                    @endforeach
                                @else
                                     <a href="{{ CDN($row->field) }}" target="_blank"> 
                                    <img class="img-responsive"
                                         src="{{ CDN($row->field) }}" width="300">
                                    </a>
                                @endif
                            ...
                            @elseif($row->type == 'file')
                                ...
                                @if(CDN_EXT($file->download_link)=='jpg' OR CDN_EXT($file->download_link)=='jpeg' OR CDN_EXT($file->download_link)=='png' OR CDN_EXT($file->download_link)=='git')
                                   
                                    <a href="{{ CDN($file->download_link) }}" target="_blank"><img src="{{ CDN($file->download_link) }}" width="300"></a>
                                   
                                @else

                                <a  href="{{ CDN($file->download_link) }}" target="_blank" >
                                {{ $file->original_name }}
                                </a>
