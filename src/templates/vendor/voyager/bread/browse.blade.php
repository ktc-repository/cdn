                                        ...
                                        @foreach($dataType->browseRows as $row)
                                            <td>
                                                ...
                                                <a  href="{{ route('voyager.'.$dataType->slug.'.show', $data->{$data->getKeyName()}) }}" >
                                                    <img src="@if( !filter_var($data->{$row->field}, FILTER_VALIDATE_URL)){{ CDN( $data->{$row->field} ) }}@else{{ CDN($data->{$row->field}) }}@endif" width="50" height="50">
                                                </a>
                                                ...
                                                @elseif($row->type == 'file' && !empty($data->{$row->field}) )
                                                    ...
                                                    @if(json_decode($data->{$row->field}))
                                                       ...
                                                       @if(CDN_EXT($file->download_link)=='jpg' OR CDN_EXT($file->download_link)=='jpeg' OR CDN_EXT($file->download_link)=='png' OR CDN_EXT($file->download_link)=='git')
                                                            <a  href="{{ route('voyager.'.$dataType->slug.'.show', $data->{$data->getKeyName()}) }}" >
                                                                <img src="{{ CDN($file->download_link) }}" width="50" height="50">
                                                            </a>
                                                        @else
                                                ...
                                                @else
                                                    ...
                                                    @if($row->type == 'multiple_images')
                                                        @php
                                                        $multiplesImages = (array) json_decode($data->{$row->field}, true);
                                                        @endphp

                                                        @foreach($multiplesImages as $item)
                                                            <a  href="{{ route('voyager.'.$dataType->slug.'.show', $data->{$data->getKeyName()}) }}" >
                                                                <img src="{{ CDN($item) }}" width="50" height="50">
                                                            </a>
                                                        @endforeach
